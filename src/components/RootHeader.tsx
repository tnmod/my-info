  ...
  return (
    <header>
      <div className='absolute top-0 left-0 w-screen z-30'>
        <div className={`w-screen items-start flex justify-between p-5 bg-transparent`}>
      ...
      >
        <div className={`w-12 h-12 md:h-14 md:w-14 flex flex-col justify-center items-center gap-1 md:gap-1.5 p-3 rounded-full ${active ? "bg-transparent" : "bg-white"}`}>
          <motion.div
            className='w-full h-0.5 md:h-1 md:rounded-full backdrop-blur-2xl '
            animate={{
              rotate: active ? 45 : 0,
              y: active ? container === "sm" || container === "md" ? 6 : 10 : 0,
          ...
          <ul className='flex flex-col h-full items-start'>
            {
              Index.map((item, index) => {
                return (
                  <li key={index} className='group/item2 py-0.5 flex items-center cursor-pointer'>
                    <a
                      onFocus={(e) => {
                        console.log(e);
                      }}
